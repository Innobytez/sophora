...
export const db = new PostgresSyncDatabase(config.databaseUrl);
...
function ensureBookingRequestConvertedEventConstraint() {
  if (constraintExists("booking_requests_converted_event_id_fkey")) return;
  db.exec(`
    ALTER TABLE booking_requests
    ADD CONSTRAINT booking_requests_converted_event_id_fkey
    FOREIGN KEY (converted_event_id)
...
function initializeSchema() {
  ...
  db.exec("ALTER TABLE artist_profiles ADD COLUMN IF NOT EXISTS availability_mode TEXT NOT NULL DEFAULT 'custom'");
  db.exec("ALTER TABLE artist_profiles ADD COLUMN IF NOT EXISTS contact_phone TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS booking_request_id BIGINT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS engagement_time TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS engagement_start_time TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS engagement_end_time TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS date_mode TEXT NOT NULL DEFAULT 'single'");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS selected_dates_json TEXT NOT NULL DEFAULT '[]'");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_name TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_email TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_phone TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_paid_at TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS artist_paid_at TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_paid_artist_acknowledged_at TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS artist_paid_artist_acknowledged_at TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_paid_artist_seen_state INTEGER NOT NULL DEFAULT 0");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS artist_paid_artist_seen_state INTEGER NOT NULL DEFAULT 0");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS require_signature INTEGER NOT NULL DEFAULT 0");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_pdf_path TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_pdf_original_name TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS signed_signature_strokes_json TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS artist_acknowledged_at TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS admin_signature_acknowledged_at TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS document_generated_at TEXT");
  db.exec("ALTER TABLE artist_events ADD COLUMN IF NOT EXISTS client_document_generated_at TEXT");
  db.exec("ALTER TABLE booking_requests ADD COLUMN IF NOT EXISTS notify_by_email INTEGER NOT NULL DEFAULT 0");
  db.exec("ALTER TABLE booking_requests ADD COLUMN IF NOT EXISTS notify_by_sms INTEGER NOT NULL DEFAULT 0");
  db.exec("ALTER TABLE booking_requests ADD COLUMN IF NOT EXISTS engagement_start_time TEXT");
  db.exec("ALTER TABLE booking_requests ADD COLUMN IF NOT EXISTS engagement_end_time TEXT");
  db.exec("ALTER TABLE booking_requests ADD COLUMN IF NOT EXISTS location TEXT");
  db.exec("ALTER TABLE booking_requests ADD COLUMN IF NOT EXISTS suggested_budget INTEGER");
  db.exec("ALTER TABLE booking_requests ADD COLUMN IF NOT EXISTS converted_event_id BIGINT");

  ensureBookingRequestConvertedEventConstraint();
}
    ...
    SET
      ...
      selected_dates_json = CASE
        WHEN selected_dates_json IS NULL OR selected_dates_json = '' THEN
          CASE
            WHEN end_date IS NOT NULL AND end_date != start_date THEN json_build_array(start_date, end_date)::text
            ELSE json_build_array(start_date)::text
          END
...
function seedArtistsIfNeeded() {
  ...
  const insertAvailability = db.prepare(`
    ...
    VALUES (?, ?, NULL, ?)
    ON CONFLICT (artist_id, available_date) DO NOTHING
  `);
